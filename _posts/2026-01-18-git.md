...
## Automation script
[gitcheck2enum](https://github.com/0xmrsecurity/ad2enum/blob/main/git/gitcheck2enum.sh)
...
```bash
# with ssl verifying
[Key Required] git clone ssh://gitea@domain.com:PORT/Repo_Name/file.git  
[Exmaple] GIT_SSH_COMMAND="ssh -i $(pwd)/id_rsa -o IdentitiesOnly=yes" git clone ssh://gitea@domain.com:PORT/Repo_Name/file.git

# without ssl verifying
git -c http.sslVerify=false git clone ssh://gitea@domain.com:PORT/Repo_Name/file.git
GIT_SSH_COMMAND="ssh -i $(pwd)/id_rsa -o IdentitiesOnly=yes" GIT_SSL_NO_VERIFY=true git clone ssh://gitea@domain.com:PORT/Repo_Name/file.git
```
...
=> [Creating New Branch]
     git checkout -b terraform-audit

  git add filename                          [add file]
  git commit -m "Commit Message"            [commit file]
  git push  <Branch_Name>                   [push to brach]
```
